...
1. **Калькулятор пути (calculator.html)** — таблица результата отображала `&nbsp;кг/м` буквально вместо неразрывного пробела. Причина: `escHtml()`, добавленный при security-hardening, двойно экранировал `&` в `&nbsp;` → `&amp;nbsp;`. Итоговая строка выглядела как `Рельс Р50 (51.67&nbsp;кг/м)`.
2. **PDF-спецификация** — inline `<style>` внутри `handleDownloadPdf()` блокировался CSP (после hardening убрали `'unsafe-inline'` из `style-src`). Печатная спецификация показывалась без стилей.
3. **Продуктовый калькулятор** — `renderPricing()` читал `item.weight_per_unit` напрямую, игнорируя уже готовую функцию `getWeightKg(item)` с fallback-таблицей ГОСТ. Позиции без `weight_per_unit` в catalog.json не получали вкладки «По метражу» / «По весу».
4. **Неверный вес Р65 в catalog.json** — скрейпер записал 811.0 kg вместо правильного по ГОСТ 8161: 64.72 кг/м × 12.5м = 809.0 kg.
5. **Отсутствующие веса DIN 536** — типы А55–А150 не имели `weight_per_unit`.

### Исправления

- `calculator.js` → заменил `&nbsp;` на `\u00a0` в конструкторе имени строки рельса (escHtml больше не ломает).
- `calculator.html` → вернул `'unsafe-inline'` в `style-src` CSP (нужен для inline-`<style>` при печати спецификации).
- `product.js` → `renderPricing()`: `item.weight_per_unit` → `getWeightKg(item)` — теперь используется fallback-таблица для позиций без weight_per_unit.
- `catalog.json` → исправлены веса 4 позиций Р65: 811.0 → 809.0; добавлены `weight_per_unit` для 6 позиций DIN 536 (А55–А150, стандарт DIN 536).
