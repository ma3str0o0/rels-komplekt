...
def _build_stats_text(days: int) -> str:
    ...
    if not db_exists():
        return (
            f"📈 <b>Аналитика</b>\n<i>обновлено {now}</i>\n\n"
            "⚠️ База метрик пока пуста.\n"
            "Данные появятся после первых визитов на сайт."
        )
    s = get_stats(days)
    lines = [
        f"📈 <b>Аналитика — {s['period']}</b>",
        f"<i>обновлено {now}</i>",
        '',
        f"👁 Просмотры: <b>{s['views']}</b> (уников: {s['unique_ips']})",
        f"📱 Заявки: <b>{s['forms']}</b>",
        f"📞 Тел.: {s['phone_clicks']}  ·  📧 Email: {s['email_clicks']}",
        f"🔢 Калькулятор: {s['calc_uses']}",
    ]
    if s['top_products']:
        lines.append('')
        lines.append('🏷 <b>Топ товары:</b>')
        for i, (pid, cnt) in enumerate(s['top_products'], 1):
            lines.append(f"  {i}. {_product_name(pid)} — {cnt}")
    if s['searches']:
        lines.append('')
        lines.append('🔍 <b>Искали:</b> ' + ', '.join(f"{q} ({n})" for q, n in s['searches']))
    return '\n'.join(lines)
...
def _build_top_text(days: int) -> str:
    ...
    lines = [f"🏷 <b>Топ-{len(items)} товаров за {label}</b>", f"<i>обновлено {now}</i>", '']
    for i, (pid, cnt) in enumerate(items, 1):
        lines.append(f"<b>{i:>2}.</b> {_product_name(pid)} — {cnt} просм.")
    return '\n'.join(lines)
